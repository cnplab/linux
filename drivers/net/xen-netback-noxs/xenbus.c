...
static int backend_create_xenvif(struct backend_info *be)
{
	...
	err = store_read_handle(dev, &handle);
	if (err != 1) {
					...
					enum xenbus_state state)
{
	...
	pr_debug("%s -> %s\n", to_xenbus_name(dev), xenbus_strstate(state));
	be->state = state;
			     ...
			     enum xenbus_state frontend_state)
{
	...
	pr_debug("%s -> %s\n", to_xenbus_otherend(dev), xenbus_strstate(frontend_state));
				...
				const char **vec, unsigned int len)
{
	...
	store_read_rate(dev, &credit_bytes, &credit_usec);
...
static int connect_ctrl_ring(struct backend_info *be)
{
	...
	if (ring_ref != INVALID_GRANT_HANDLE) {
...
static void connect(struct backend_info *be)
{
	...
	err = store_read_mac(dev, be->vif->fe_dev_addr);
	if (err) {
		xenbus_dev_fatal(dev, err, "parsing %s/mac", to_xenbus_name(dev));
		return;
	}
